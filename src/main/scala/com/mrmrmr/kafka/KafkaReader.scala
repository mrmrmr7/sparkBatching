...
object KafkaReader {
  def getHotelWithWeatherDF(session: SparkSession): DataFrame = {
    ...
    session
      ...
      .option("startingOffsets", session.sparkContext.getConf.get("spark.offsets.starting"))
      .option("maxOffsetsPerTrigger", session.sparkContext.getConf.get("spark.offsets.max.per.trigger"))
      .load()
